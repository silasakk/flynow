...
class request_service{
    ...
    {
        add_menu_page(
            __("Request service", $this->name), 
            __("Request service", $this->name), 
            'activate_plugins', 
            'request_services', 
            array($this,'add_page_request_service'),
            "dashicons-admin-tools",
            8
        );
        add_submenu_page(
            'request_services', 
            __("All Request service", $this->name), 
            __("All Request service", $this->name), 
            'activate_plugins',
            'request_services', 
            array($this,'add_page_request_service')
        ...
        add_submenu_page(
            ...
            __('Add new', $this->name), 
            'activate_plugins', 
            'add_form_request_service', 
            array($this,'add_form_request_service')
        );
    }
    ...
    public function install(){
        ...
        $installed_ver = get_option('request_service_version');
        ...
        if ($installed_ver != $this->version) {
            ...
            update_option('request_service_version', $this->version);
        }
    ...
    function add_page_request_service()
    {
    ...
    function add_form_request_service()
    {
        global $wpdb;
        ...
        if (isset($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'],  $this->name)) {
            ...
            $uploadedfile = $_FILES['attactment'];
        ...
        add_meta_box('form_meta_box', 'Form data', array($this,'form_meta_box'), 'add_form_request_service', 'normal', 'default');
    ...
    function validate($item){
        ...
        if (empty($item['fullname'])) 
            $messages[] = __('Name is required', 'request_service');

        if (empty($item['phone'])) 
            $messages[] = __('Phone is required', 'request_service');

        /*if (!empty($item['category'])) 
        $messages[] = __('Category is required', 'request_service');

    if (!empty($item['product'])) 
        $messages[] = __('Product is required', 'request_service');

    if (!empty($item['series'])) 
        $messages[] = __('Series is required', 'request_service');*/
